...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, loc_preds, loc_targets, conf_preds, conf_targets):
        ...
        conf_loss = self.cross_entropy_loss(conf_preds.view(-1,self.num_classes), \
                                            conf_targets.view(-1))  # [N*1940,]
        neg = self.hard_negative_mining(conf_loss, pos)    # [N,1940]

        pos_mask = pos.unsqueeze(2).expand_as(conf_preds)  # [N,1940,21]
        neg_mask = neg.unsqueeze(2).expand_as(conf_preds)  # [N,1940,21]
